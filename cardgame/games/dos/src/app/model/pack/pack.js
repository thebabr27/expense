class Pack {
        ...
        for (let k=0;k<2; k++) {
            ...
            for (let j in specialCards) {
                ...
                let newCard = new Card()
                    newCard.create({
                        id: count
                        , icon: `black-card ${specialCards[j].name}`
    ...
    transferCardsTo(num,pack) { //cut and paste cards from this pack to another
        if (typeof(num)=='object' && num.length && num.length>0) { 
            pack.giveCards(this.selectCardsByIds(num))            
            console.log("è un Array",this,pack,num) 
